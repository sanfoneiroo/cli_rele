*
  Projeto: CLI do Relé (versão Active-Low)

  Este código é uma variação do "cli_rele.ino", adaptado para módulos de relé
  que funcionam em lógica invertida (active-low).

  Diferença principal:
  - No módulo active-low, o relé é acionado quando recebe nível LOW.
  - Isso garante que, caso o Arduino esteja desligado (sem energia),
    o relé permanece DESLIGADO, evitando que a carga fique ligada
    de forma indesejada em caso de falha de energia.

  Observação:
  Use esta versão se o seu módulo de relé estiver acionando sozinho
  quando o Arduino não está energizado.
*/
